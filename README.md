...
![Tests](https://img.shields.io/badge/tests-164%20passing-22c98a)
...
| **Tests** | pytest — **164 tests, all passing** | Runs the real graph with no services and no keys |
...
- [x] **Path option** — learners state a career/skill goal at `/path` (nav **Path**); hybrid retrieval + Mesh (or heuristic fallback) sequences real catalog courses; goal is saved as `User.career_goal` and biases the recommendation agent; JSON at `GET/POST /api/path`
